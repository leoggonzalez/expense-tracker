...
- Components under `src/components/*` are encapsulated UI units and must not expose `style` or `className` props.
...
- This rule applies to `src/components/*` only. Shared primitives under `src/elements/*` remain configurable by design.
...
- A reviewer should be able to reject any new `style` or `className` prop added to a component under `src/components/*`.
