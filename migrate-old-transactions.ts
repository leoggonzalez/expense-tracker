...
async function main() {
  ...
  const uniqueSpaces = [...new Set(oldTransactions.map((transaction) => transaction.group))];
  console.log(
  ...
  // Step 3: Create transactions with the new schema
  let created = 0;
  let skipped = 0;

  for (const transaction of oldTransactions) {
    const spaceId = spaceMap.get(transaction.group);
    
    if (!spaceId) {
      console.log(`Skipping transaction ${transaction.id}: space not found`);
      skipped++;
      continue;
    }
    ...
    try {
      ...
      if (existing) {
        console.log(`Transaction already exists: ${transaction.description} (${transaction.id})`);
        skipped++;
        continue;
      }
      ...
      console.log(`✓ Created: ${transaction.description} (${transaction.group})`);
      created++;
    } catch (error) {
      console.error(`✗ Error creating transaction ${transaction.id}:`, error);
      skipped++;
    }
  }
  ...
  console.log(`Total transactions processed: ${oldTransactions.length}`);
}
